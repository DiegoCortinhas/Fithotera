...
def procuraRegex(produto_final,contador):
    if(produto_final.__contains__("Origem:")):
        regex_base = "(:\s)([A-zÀ-ÿ0-9 \/]+)(|&)"
        if (re.findall(regex_base, produto_final) != []):
            descricao.append(re.findall(regex_base, produto_final))
            print(contador)
            nomeBotanico = str(descricao[contador][0][1])
            origem = str(descricao[contador][1][1])
            dados_produto = (nomeBotanico, origem, produto_final)
...
for i in range (1):
    ...
    for ahref in links:
        ...
        if (not href.__contains__("kit")):
            ...
            if not produto_c_tag:
                produto_final = removeTagProduto(produto_c_tag2)
                procuraRegex(produto_final,contador)
                             

            else:
                produto_final = removeTagProduto(produto_c_tag)
